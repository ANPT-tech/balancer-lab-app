...
|---|---|
| Ответ должен позволять определить backend-ноду | `GET /api/backend-info` возвращает `instance_id`, `hostname`, `pid`, `request_id`. Каждый ответ содержит `X-Backend-Node` |
| Не должно возникать проблем с хранилищем при отказе одной ноды | Приложение stateless и не хранит пользовательские данные. Локальная БД не используется |
| Приложение не должно самостоятельно обрабатывать TLS/SSL | Uvicorn работает только по HTTP. TLS завершается на Nginx |
| Сессии нельзя хранить в RAM или локальных файлах | Пользовательские сессии не используются |
| Нужно два и более одинаковых экземпляра | Один Docker image запускается как `backend-1` и `backend-2` |
| Нужна асинхронная обработка | FastAPI endpoints используют `async def`, `/api/async-work` выполняет неблокирующий `await asyncio.sleep(...)` |
...
1. Две VM: `192.168.52.136` и `192.168.52.137`.
2. `docker ps` и два backend-контейнера.
3. Прямые ответы `backend-1` и `backend-2`.
4. HTTPS-запросы через Nginx со сменой `instance_id`.
5. Заголовок `X-Backend-Node`.
6. Сайт `https://app.lab.test`.
7. «30 параллельных запросов».
8. L4-балансировку через `:9000`.
9. `Resolve-DnsName app.lab.test` с двумя A-записями.
10. Остановку одной backend-ноды и продолжение работы.
11. `ss -lntp`, подтверждающий, что TLS слушает Nginx, а backend работает по HTTP.

---

## Рекомендуемые скриншоты

- главная страница `https://app.lab.test`;
- результат «30 параллельных запросов»;
- `Resolve-DnsName app.lab.test` с двумя IP;
- L7-запросы с чередованием `backend-1` / `backend-2`;
- L4-запросы через `:9000`;
- `ss -lntp` с портами `443`, `80`, `9000`, `8001`, `8002`;
- работа приложения после остановки одного backend.
...
Перед сдачей рекомендуется также добавить копии инфраструктурных конфигураций:
...
Это сделает репозиторий полностью воспроизводимым.
...
- приватные TLS-ключи (`*.key`);
- пароли;
- SSH-ключи;
- секреты.

Конфигурацию Nginx хранить можно, но приватный ключ сертификата лучше генерировать при развёртывании.
...
git init
git add .
git commit -m "Complete load balancing lab"
git branch -M main
git remote add origin <URL_РЕПОЗИТОРИЯ>
git push -u origin main
```

В СДО прикладывается ссылка на репозиторий с исходным кодом.
